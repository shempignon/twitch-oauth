...
pub async fn get_app_access_token(client_id: &str, client_secret: &str, scopes: Vec<String>) -> Result<AppAccessToken, Box<dyn std::error::Error>> {
...
pub async fn validate_token(token: AppAccessToken) -> Result<ValidatedToken, Box<dyn std::error::Error>> {
...
pub async fn revoke_token(token: AppAccessToken, client_id: &str) -> Result<(), Box<dyn std::error::Error>> {
  let mut params = HashMap::new();
